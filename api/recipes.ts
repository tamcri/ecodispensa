...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (idea) {
    ...
    const { data: profile, error: profileErr } = await supabase
      .from("user_profiles")
      .select("diet, lactose_free, avoid, allergies, default_servings, max_time_minutes_default, plan_type, premium_until")
      .eq("user_id", user_id)
      .maybeSingle();
...
MODALITÀ DI LAVORO:
${idea
  ? `RICERCA RICETTA SPECIFICA: l'utente ha chiesto "${idea}". Devi generare SOLO quella ricetta, nella versione corretta e riconoscibile. NON devi adattare il piatto agli ingredienti della dispensa se questo lo renderebbe sbagliato.`
  : "DISPENSA ANTI-SPRECO: devi suggerire 3 ricette usando soprattutto ingredienti disponibili, senza mai usare scaduti."}

OBIETTIVO:
${idea
  ? `- Genera UNA SOLA ricetta completa, credibile e coerente per: "${idea}".`
  : "- Suggerisci 3 ricette gustose usando soprattutto gli ingredienti disponibili e non scaduti."}
- Titolo, descrizione, ingredienti e passaggi devono appartenere allo stesso piatto.
- La ricetta deve essere realistica: una persona deve poterla cucinare davvero seguendo i passaggi.
- Non generare ingredienti incompatibili con il tipo di piatto.
- Non mescolare dolce e salato in modo incoerente.
...
REGOLE SPECIALI PER "CERCA RICETTA":
- Se l'utente chiede un piatto specifico, devi rispettare quel piatto.
- NON trasformare un dolce in una ricetta salata.
- NON trasformare un piatto tradizionale in una variante casuale solo perché certi ingredienti sono in dispensa.
- Se un ingrediente fondamentale non è in dispensa, NON sostituirlo con ingredienti incompatibili: mettilo in "missingIngredients".
- Esempio: se l'utente chiede "Crema catalana", gli ingredienti coerenti sono latte, tuorli, zucchero, amido, limone, cannella, vaniglia; NON usare pomodoro, mozzarella, guanciale, pasta, formaggio o verdure.
- Esempio: se l'utente chiede "Torta di mele", usa ingredienti da dolce come mele, farina, zucchero, uova, latte o alternative compatibili; NON usare ingredienti salati.
- Esempio: se l'utente chiede "Spaghetti alle vongole", usa spaghetti o linguine, vongole, aglio se consentito, olio, prezzemolo; NON proporre altri piatti.
- La dispensa serve solo per capire cosa l'utente ha già. Non deve mai rovinare la coerenza della ricetta richiesta.

REGOLE DIETETICHE E SICUREZZA:
...
- Se diet = vegan: niente carne, pesce, uova, latte, formaggi, burro, yogurt, miele o altri ingredienti animali.
...
- Se la dispensa è vuota, "ingredientsUsed" deve essere [] e gli ingredienti necessari devono stare in "missingIngredients".

REGOLE DI QUALITÀ INGREDIENTI:
- Usa ingredienti specifici e coerenti.
- Non usare mai: "pasta avanzata", "pasta a scelta", "formaggio a piacere", "verdure miste", "ingredienti vari", "condimento pronto".
- Per la pasta specifica sempre il formato: spaghetti, penne, rigatoni, fusilli, linguine, tagliatelle, ecc.
- Per il riso specifica il tipo quando rilevante: Carnaroli, Arborio, basmati, originario, ecc.
- Per i dolci usa solo ingredienti coerenti da pasticceria, salvo richieste esplicite diverse.
- Per piatti salati usa ingredienti coerenti con cucina salata.
- "missingIngredients" deve includere quantità indicative, ad esempio "300 g farina 00", "4 tuorli", "500 ml latte senza lattosio".

REGOLE SUI PASSAGGI:
- "steps" deve contenere almeno 7 passaggi pratici e ordinati.
- Ogni passaggio deve essere autonomo, concreto e utile.
- Specifica tempi, fiamma, temperatura, consistenza o segnali visivi quando serve.
- Non usare frasi generiche tipo "prepara la ricetta", "cuoci tutto" o "mescola gli ingredienti" senza spiegare come.
- L'utente deve poter cucinare seguendo solo i passaggi.
- Per dolci: indica preparazione, incorporazione ingredienti, cottura/raffreddamento e finitura.
- Per pasta: indica acqua, sale, cottura al dente, condimento e mantecatura.
- Per risotti: indica tostatura, aggiunta graduale brodo, cottura e mantecatura.
- L'ultimo passaggio deve spiegare come completare, impiattare o servire.

CONTROLLO FINALE PRIMA DI RISPONDERE:
- Verifica che nessun ingrediente sia incompatibile con il titolo.
- Verifica che un dolce non contenga ingredienti salati incoerenti.
- Verifica che un piatto specifico richiesto non sia diventato un'altra ricetta.
- Verifica che "ingredientsUsed" contenga solo ingredienti davvero presenti in dispensa.
- Verifica che tutti gli ingredienti necessari mancanti siano in "missingIngredients".
