...
import { ChefHat, Clock, BarChart, Loader2, Sparkles, AlertTriangle, Search, X, ChevronRight, CheckCircle2, Utensils } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="flex p-1 bg-gray-100 rounded-xl">
          <button 
            onClick={() => { setMode('suggest'); setRecipes([]); setError(null); }}
            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${mode === 'suggest' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
              Dispensa Anti-Spreco
          </button>
          <button 
             onClick={() => { setMode('search'); setRecipes([]); setError(null); }}
             className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${mode === 'search' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
              Cerca & Pianifica
          </button>
      </div>
          ...
          >
            <div className="p-5">
              <div className="flex justify-between items-start mb-2">
                <h3 className="text-xl font-bold text-gray-800 leading-tight group-hover:text-emerald-700 transition-colors">{recipe.title}</h3>
                <span className={`text-[10px] px-2 py-1 rounded-full uppercase font-bold tracking-wide border
                    ${recipe.difficulty === 'Facile' ? 'bg-green-50 text-green-700 border-green-100' :
                      recipe.difficulty === 'Media' ? 'bg-yellow-50 text-yellow-700 border-yellow-100' :
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500 mb-4">
                 ...
                 <span className="flex items-center gap-1"><BarChart size={14} /> {recipe.ingredientsUsed.length} ingr. usati</span>
              </div>
                  ...
                  <div className="flex-1">
                       <div className="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
                           <div 
                                ...
                                style={{ width: `${(recipe.ingredientsUsed.length / (recipe.ingredientsUsed.length + recipe.missingIngredients.length)) * 100}%` }}
                            />
                       </div>
              ...
              <div className="bg-white w-full max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[90vh] overflow-y-auto animate-slide-up flex flex-col shadow-2xl">
                  ...
                  <div className="p-6 space-y-6 flex-1 overflow-y-auto">
                      ...
                      <div className="flex gap-4 text-sm text-gray-500 border-b border-gray-50 pb-4">
                          <span className="flex items-center gap-1 bg-gray-50 px-3 py-1 rounded-lg"><Clock size={16} /> {selectedRecipe.time}</span>
                          <span className="flex items-center gap-1 bg-gray-50 px-3 py-1 rounded-lg"><BarChart size={16} /> {selectedRecipe.difficulty}</span>
                      </div>
